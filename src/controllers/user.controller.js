...
const register = async (req, res) => {
    try {
        ...
        // Naya user banao
        const newUser = new User({ name, mobile, cnic, email, pin: hashedPin, balance: 25000 });
        await newUser.save();
...
const addTransaction = async (req, res) => {
    try {
        ...
        const limit = user.transactionLimit || (user.accountType === 'verified' ? 500000 : 25000);
        if ((type === 'sent' || type === 'bill') && amount > limit) {
            return res.status(400).json({ message: `Transaction limit exceeded. Your limit is Rs ${limit.toLocaleString()}` });
        }
        ...
        if (type === 'sent' && recipient && recipient.length === 11 && recipient.startsWith('03')) {
...
const addRecurring = async (req, res) => {
    try {
        ...
        user.recurringPayments.push({ title, amount, schedule, date, active: true });
        await user.save();

        res.json({ message: 'Recurring payment added', recurringPayments: user.recurringPayments });
...
const toggleRecurring = async (req, res) => {
    try {
        ...
        res.json({ message: 'Recurring payment toggled', recurringPayments: user.recurringPayments });
...
const redeemCashback = async (req, res) => {
    try {
        ...
        if (user.cashbackPoints < 100) {
            return res.status(400).json({ message: 'Need at least 100 points to redeem' });
        }
